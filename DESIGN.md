...
**2026-08-01 run — regenerated brand slots plus audio.** Every raster motif the live landing ships was rebuilt in the clay-voxel language with `google/gemini-3.1-flash-image`: the bottle-cap coin (`img-coin`), treasure mound (`img-treasure`), gamepad (`img-gamepad`), crowd (`img-users`), gift box (`img-gift`), app-in-phone (`img-phone`), and up/down tokens (`img-arrows`). They ship as DOM illustration in glass frames; chrome stays vector. `coin-spin.mp4` (`google/veo-3.1-lite`) animates the coin for the onboarding in a circular crop.

Audio is generated, not synthesized by hand: `google/lyria-3-clip-preview` renders 30-second 48 kHz clips at `$0.04`. `music-ambient.mp3` is a dark lounge-ambient bed closed into a seamless 27.8-second loop by WAV-domain envelope+amix blending (acrossfade proved unreliable at mp3 EOF). Six one-shots (`sfx-commit`, `sfx-coin`, `sfx-tick`, `sfx-whoosh`, `sfx-win`, `sfx-lose`) are onset-trimmed from sparse sound-design clips and fire as UI feedback under the bed. Total recorded spend across all runs is in `public/media/manifest.json` (`$4.35` at this writing).
